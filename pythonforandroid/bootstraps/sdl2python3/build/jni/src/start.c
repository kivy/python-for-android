...
int main(int argc, char *argv[]) {
 ...
 /* AND: P4A uses env vars...not sure what's best */
    ...
    /* Py_SetProgramName(argv[0]);  /\* Disabled due to problem passing wchar_ *\/ */
    Py_SetProgramName("testsetprogramname");
    LOG("Set program name");
    Py_Initialize();
    /* PySys_SetArgv(argc, argv); */  /* Disabled due to problem passing wchar_ */
