...
class Context(object):
    ...
    def setup_dirs(self, storage_dir):
        ...
        if ' ' in self.storage_dir:
            raise ValueError(
                'storage dir path cannot contain spaces, please '
    ...
    def android_api(self):
        ...
        if self._android_api is None:
            raise ValueError(
                'Tried to access android_api but it has not '
    ...
    def ndk_ver(self):
        ...
        if self._ndk_ver is None:
            raise ValueError(
                'Tried to access ndk_ver but it has not '
                'been set - this should not happen, something '
                'went wrong!'
            )
    ...
    def sdk_dir(self):
        ...
        if self._sdk_dir is None:
            raise ValueError(
                'Tried to access sdk_dir but it has not '
                'been set - this should not happen, something '
                'went wrong!'
            )
    ...
    def ndk_dir(self):
        ...
        if self._ndk_dir is None:
            raise ValueError(
                'Tried to access ndk_dir but it has not '
                'been set - this should not happen, something '
                'went wrong!'
            )
    ...
    def prepare_build_environment(
        self, user_sdk_dir, user_ndk_dir, user_android_api, user_ndk_ver
    ):
        ...
        if sdk_dir is None:
            possible_dirs = glob.glob(
                expanduser(
                    join('~', '.buildozer', 'android', 'platform', 'android-sdk-*')
                )
            )
            possible_dirs = [
                d for d in possible_dirs if not (d.endswith('.bz2') or d.endswith('.gz'))
            ]
            if possible_dirs:
                info(
                    'Found possible SDK dirs in buildozer dir: {}'.format(
                        ', '.join([d.split(os.sep)[-1] for d in possible_dirs])
                    )
                )
                info('Will attempt to use SDK at {}'.format(possible_dirs[0]))
                warning(
                    'This SDK lookup is intended for debug only, if you '
        ...
        if android_api is None:
            info(
                'Android API target was not set manually, using '
        ...
        if exists(join(sdk_dir, 'tools', 'bin', 'avdmanager')):
            ...
            error(
                'Could not find `android` or `sdkmanager` binaries in '
                'Android SDK. Exiting.'
            )
            exit(1)
        ...
        if android_api in apis:
            info(
                ('Requested API target {} is available, ' 'continuing.').format(
                    android_api
                )
            )
        else:
        ...
        if ndk_dir is None:  # Checks in the buildozer NDK dir, useful
            ...
            possible_dirs = glob.glob(
                expanduser(
                    join('~', '.buildozer', 'android', 'platform', 'android-ndk-r*')
                )
            )
            if possible_dirs:
                info(
                    'Found possible NDK dirs in buildozer dir: {}'.format(
                        ', '.join([d.split(os.sep)[-1] for d in possible_dirs])
                    )
                )
                info('Will attempt to use NDK at {}'.format(possible_dirs[0]))
                warning(
                    'This NDK lookup is intended for debug only, if you '
                    'use python-for-android much you should probably '
        ...
        try:
            ...
            if ndk_ver is None:
                ...
                info(
                    ('Got Android NDK version from the NDK dir: ' 'it is {}').format(
                        ndk_ver
                    )
                )
            else:
                if ndk_ver != reported_ndk_ver:
                    warning(
                        'NDK version was set as {}, but checking '
                        'the NDK dir claims it is {}.'.format(ndk_ver, reported_ndk_ver)
                    )
                    warning(
                        'The build will try to continue, but it may '
                        'fail and you should check '
                        'that your setting is correct.'
                    )
                    warning(
                        'If the NDK dir result is correct, you don\'t '
                        'need to manually set the NDK ver.'
                    )
        if ndk_ver is None:
            warning(
                'Android NDK version could not be found. This probably'
        ...
        self.ndk_platform = join(
            self.ndk_dir, 'platforms', 'android-{}'.format(self.android_api), platform_dir
        )
        ...
        if isdir(toolchain_path):
            toolchain_contents = glob.glob(
                '{}/{}-*'.format(toolchain_path, toolchain_prefix)
            )
            toolchain_versions = [
        ...
        if toolchain_versions:
            ...
            info(
                'Picking the latest gcc toolchain, here {}'.format(
        ...
        for executable in (
            ...
            if not sh.which(executable):
                warning("Missing executable: {} is not installed".format(executable))

        if not ok:
            error(
                '{}python-for-android cannot continue; aborting{}'.format(
    ...
    def __init__(self):
        ...
        self.archs = (ArchARM(self), ArchARMv7_a(self), Archx86(self), ArchAarch_64(self))
    ...
    def set_archs(self, arch_names):
        ...
        if not self.archs:
            ...
            exit(1)
        info(
            'Will compile for the following archs: {}'.format(
    ...
    def has_package(self, name, arch=None):
        ...
        return (
            exists(join(site_packages_dir, name))
...
def run_pymodules_install(ctx, modules):
    ...
    info(
        'The requirements ({}) don\'t have recipes, attempting to install '
    ...
    with current_directory(join(ctx.build_dir)):
        ...
        info(
            'If this fails with a message about /bin/false, this '
...
def biglink(ctx, arch):
    ...
    for recipe in recipes:
        recipe_obj_dir = join(
            recipe.get_build_container_dir(arch.arch), 'objects_{}'.format(recipe.name)
        )
        ...
        for fn in os.listdir(directory):
            ...
            with open(fn) as f:
                ...
                needed_libs = [
                    lib
                    for lib in libs
                    if lib and lib not in blacklist_libs and lib not in found_libs
                ]
