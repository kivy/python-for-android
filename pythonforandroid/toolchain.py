...
def check_python_dependencies():
    ...
    modules = [('colorama', '0.3.3'), 'appdirs', ('sh', '1.10'), 'jinja2', 'six']
        ...
        try:
            ...
            if version is None:
                ...
                print(
                    ...
                    'please install version {} or higher'.format(module, version)
                )
                ok = False
        else:
            if version is None:
...
def add_boolean_option(
    parser, names, no_names=None, default=True, dest=None, description=None
):
    ...
    group.add_argument(
        *opts,
        help=("(this is the default)" if default else None),
        dest=dest,
        action='store_true'
    )
    if no_names is None:
    ...
    group.add_argument(
        *opts,
        help=(None if default else "(this is the default)"),
        dest=dest,
        action='store_false'
    )
...
def build_dist_from_args(ctx, dist, args):
    ...
    info_notify(
        'Dist will have name {} and recipes ({})'.format(
            dist.name, ', '.join(dist.recipes)
        )
    )
    info(
        'Dist will also contain modules ({}) installed from pip'.format(
...
class ToolchainCL(object):
    def __init__(self):
        ...
        generic_parser = argparse.ArgumentParser(
            add_help=False, description='Generic arguments applied to all commands'
        )
        ...
        generic_parser.add_argument(
            ...
            default='',
            help=(
                'The version of the Android NDK. This is optional: '
        ...
        generic_parser.add_argument(
            '--requirements',
            help=(
                'Dependencies of your app, should be recipe names or ' 'Python modules'
            ),
            default='',
        )
        ...
        add_boolean_option(
            ...
            default=False,
            description=(
                'Whether the dist recipes must perfectly match ' 'those requested'
            ),
        )
        ...
        subparsers = parser.add_subparsers(
            dest='subparser_name', help='The command to run'
        )
        ...
        parser_clean = add_parser(
            subparsers, 'clean', help='Delete build components.', parents=[generic_parser]
        )
        parser_clean.add_argument(
            'component',
            nargs='+',
            help=(
                'The build component(s) to delete. You can pass any '
        ...
        parser_clean_recipe_build = add_parser(
            ...
            help=(
                'Delete the build components of the given recipe. '
        ...
        parser_export_dist.add_argument(
            '--symlink', action='store_true', help='Symlink the dist instead of copying'
        )

        parser_apk = add_parser(
            subparsers, 'apk', help='Build an APK', parents=[generic_parser]
        )
        ...
        parser_apk.add_argument(
            ...
            default=None,
            help=(
                'Keystore for JAR signing key, will use jarsigner '
        ...
        parser_adb = add_parser(
            subparsers, 'adb', help='Run adb from the given SDK', parents=[generic_parser]
        )
        ...
        if args.compact:
            ...
            for name in sorted(Recipe.list_recipes(ctx)):
                try:
                    ...
                    warning(
                        (
                            'Recipe "{}" could not be loaded due to a ' 'syntax error'
                        ).format(name)
                    )
                version = str(recipe.version)
                print(
                    '{Fore.BLUE}{Style.BRIGHT}{recipe.name:<12} '
                    '{Style.RESET_ALL}{Fore.LIGHTBLUE_EX}'
                    '{version:<8}{Style.RESET_ALL}'.format(
                        recipe=recipe, Fore=Out_Fore, Style=Out_Style, version=version
                    )
                )
                print(
                    '    {Fore.GREEN}depends: {recipe.depends}'
                    '{Fore.RESET}'.format(recipe=recipe, Fore=Out_Fore)
                )
                if recipe.conflicts:
                    print(
                        '    {Fore.RED}conflicts: {recipe.conflicts}'
                        '{Fore.RESET}'.format(recipe=recipe, Fore=Out_Fore)
                    )
                if recipe.opt_depends:
                    print(
                        '    {Fore.YELLOW}optional depends: '
    ...
    def bootstraps(self, _args):
        ...
        for bs in Bootstrap.list_bootstraps():
            ...
            print(
                '{Fore.BLUE}{Style.BRIGHT}{bs.name}{Style.RESET_ALL}'.format(
                    bs=bs, Fore=Out_Fore, Style=Out_Style
                )
            )
            print(
                '    {Fore.GREEN}depends: {bs.recipe_depends}{Fore.RESET}'.format(
                    bs=bs, Fore=Out_Fore
                )
            )
    ...
    def apk(self, args):
        ...
        with current_directory(dist.dist_dir):
            ...
            if build_type == 'auto':
                ...
                info(
                    ('Detected highest available build tools ' 'version to be {}').format(
                        build_tools_version
                    )
                )
                ...
                if build_tools_version >= '25.0' and exists('gradlew'):
                    ...
                    if build_tools_version < '25.0':
                        info(
                            (
                                '    Building with ant, as the highest '
            ...
            if build_type == 'gradle':
                ...
                if args.build_mode == "debug":
                    ...
                    error("Unknown build mode {} for apk()".format(args.build_mode))
                    exit(1)
        ...
        if not apk_file:
            ...
            for suffix in suffixes:
                ...
                if apks:
                    if len(apks) > 1:
                        info(
                            'More than one built APK found... guessing you '
        ...
        if apk_add_version:
            ...
            apk_file_dest = "{}-{}-{}".format(apk_name, build_args.version, apk_suffix)
    ...
    def archs(self, _args):
        ...
        print(
            '{Style.BRIGHT}Available target architectures are:'
    ...
    def distributions(self, _args):
        ...
        if dists:
            ...
            print(
                '{Style.BRIGHT}There are no dists currently built.'
                '{Style.RESET_ALL}'.format(Style=Out_Style)
            )
    ...
    def delete_dist(self, _args):
        ...
        if dist.needs_build:
            info(
                'No dist exists that matches your specifications, '
    ...
    def build_status(self, _args):
        ...
        print(
            '{Style.BRIGHT}Recipes that are probably already built:'
            '{Style.RESET_ALL}'.format(Style=Out_Style)
        )
        ...
        if exists(other_builds_dir):
            for filen in sorted(os.listdir(other_builds_dir)):
                ...
                recipe_str = (
                    '    {Style.BRIGHT}{Fore.GREEN}{name}'
                    '{Style.RESET_ALL}'.format(Style=Out_Style, name=name, Fore=Out_Fore)
                )
                if dependencies:
                    recipe_str += (
                        ' ({Fore.BLUE}with ' + ', '.join(dependencies) + '{Fore.RESET})'
                    ).format(Fore=Out_Fore)
