...
class ToolchainCL(object):
    ...
    def __init__(self):
        ...
        parser_clean_download_cache = add_parser(
            subparsers, 'clean_download_cache',
        ...
        parser_apk.add_argument(
            '--release',
            dest='build_mode', action='store_const',
            const='release', default='debug',
            help='Build the PARSER_APK. in Release mode')
        parser_apk.add_argument(
            '--keystore',
            dest='keystore', action='store', default=None,
            help='Keystore for JAR signing key, will use jarsigner '
                 'by default if not specified (release build only)')
        parser_apk.add_argument(
            '--signkey',
            dest='signkey', action='store', default=None,
            help='Key alias to sign PARSER_APK. with (release build only)')
        parser_apk.add_argument(
            '--keystorepw',
            dest='keystorepw', action='store', default=None,
            help='Password for keystore')
        parser_apk.add_argument(
            '--signkeypw',
            dest='signkeypw', action='store', default=None,
        ...
        add_parser(
            ...
            parents=[generic_parser])
        add_parser(
            subparsers, 'distributions',
            aliases=['dists'],
            help='List the currently available (compiled) dists',
            parents=[generic_parser])
        add_parser(
            subparsers, 'delete_dist',
            aliases=['delete-dist'], help='Delete a compiled dist',
            parents=[generic_parser])

        parser_sdk_tools = add_parser(
            subparsers, 'sdk_tools',
            aliases=['sdk-tools'],
            help='Run the given binary from the SDK tools dist',
            parents=[generic_parser])
